...
class OutlookToolBase(BaseTool):
    """Base class for Outlook tools, handling connection and credentials."""

    class Config:
        arbitrary_types_allowed = True

    email_address: Optional[str] = Field(None, description="Outlook email address")
    access_token: Optional[str] = Field(None, description="Outlook app password")

    def __init__(self, description: str = ""):
        super().__init__(description=description)
        self.access_token = os.environ.get("CLIENTID")
        self.email_address = os.environ.get("email")

        if not self.email_address or not self.access_token:
            raise ValueError("EMAIL_ADDRESS and ACCESS_TOKEN must be set in the environment.")

    def _connect(self):
        """Connect to Microsoft Graph API."""
        try:
            print(f"Connecting to Graph API with email: {self.email_address[:3]}...{self.email_address[-8:]}")
            ...
            if response.status_code == 200:
                ...
                print(f"Successfully connected to Graph API for user: {user_info.get('displayName', 'Unknown')}")
                return headers  # Return headers for future API calls
            else:
                raise Exception(f"Graph API connection failed: {response.status_code} - {response.text}")
    ...
    def _disconnect(self, connection):
        """Disconnect from Graph API (no action needed)."""
        print("Graph API connection closed")
        pass

    def _get_thread_messages(self, headers: Dict, folder: str = "inbox", filter_query: str = None, limit: int = 5) -> List[
        Dict]:
        try:
            # Build the URL
            ...
            print(f"Error in _get_messages: {e}")
            return []
...
class GetUnreadEmailsTool(OutlookToolBase):
    """Tool to get unread emails from Outlook."""
    name: str = "get_unread_emails"
    description: str = "Gets unread emails from Outlook"
    args_schema: Type[BaseModel] = GetUnreadEmailsSchema

    def _run(self, limit: Optional[int] = 5) -> List[Tuple[str, str, str, str, Dict]]:
        mail = self._connect()
        try:
            print("DEBUG: Connecting to Outlook...")
            mail.select("INBOX")
            ...
            for i, email_id in enumerate(email_ids):
                ...
                print(
                    f"DEBUG: Email tuple structure: subject={subject}, sender={sender}, body_length={len(full_body)}, email_id={email_id.decode('utf-8')}, thread_info_keys={thread_info.keys()}")
...
class SaveDraftTool(BaseTool):
    ...
    description: str = "Saves an email as a draft in Outlook"
    ...
    def _connect(self):
        ...
        access_token = os.environ.get("CLIENTID")
        email_address = os.environ.get("email")

        if not email_address or not access_token:
            raise ValueError("EMAIL_ADDRESS or ACCESS_TOKEN environment variables not set")
    ...
    def _run(self, subject: str, body: str, recipient: str, thread_info: Optional[Dict[str, Any]] = None) -> str:
        try:
            ...
            if verified:
                ...
                # Try Outlook's API approach as a fallback
                try:
                    # Try saving directly to All Mail and flagging as draft
                    result, data = mail.append('[Outlook]/All Mail', '\\Draft', date, message.as_bytes())
                    if result == 'OK':
                        return f"Draft saved to All Mail with subject: '{subject}' (flagged as draft)"
                    else:
                        return f"WARNING: Draft save attempt returned {result}, but verification failed. Please check your Outlook Drafts folder."
                except Exception as e:
...
class OutlookOrganizeSchema(BaseModel):
    """Schema for OutlookOrganizeTool input."""
...
class OutlookOrganizeTool(OutlookToolBase):
    """Tool to organize emails based on agent categorization."""
    name: str = "organize_email"
    description: str = "Organizes emails using Outlook's priority features based on category and priority"
    args_schema: Type[BaseModel] = OutlookOrganizeSchema

    def _run(self, email_id: str, category: str, priority: str, should_star: bool = False,
        ...
        print(
            f"Organizing email {email_id} with category {category}, priority {priority}, star={should_star}, labels={labels}")
...
class OutlookDeleteSchema(BaseModel):
    """Schema for OutlookDeleteTool input."""
...
class OutlookDeleteTool(BaseTool):
    ...
    description: str = "Deletes an email from Outlook"
...
class EmptyTrashTool(BaseTool):
    """Tool to empty Outlook trash."""
    name: str = "empty_outlook_trash"
    description: str = "Empties the Outlook trash folder to free up space"
    ...
    def _run(self) -> str:
        """Empty the Outlook trash folder."""
        try:
            mail = self._connect()

            # Try different trash folder names (Outlook can have different naming conventions)
            trash_folders = [
                '"[Outlook]/Trash"',
                '[Outlook]/Trash',
                'Trash',
            ...
            if success:
                return f"Successfully emptied Outlook trash folder ({trash_folder_used}). Deleted {count} messages."
            else:
